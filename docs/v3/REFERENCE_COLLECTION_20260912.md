...
- 案例来源、模型、画师、LoRA 与内容筛选选中即生效，可一键清除。文字搜索在输入停顿 300 毫秒后应用，也支持回车或“搜索 / 刷新”立即执行。
- 参考列表使用页面滚动，接近底部自动载入下一页，显示已载入数与筛选后的总数；保留“加载更多”和失败重试。筛选变化会取消旧请求、重置游标，保留正在查看的详情与未保存编辑。
...
2026-09-17 更新：正常启动应用后，会在后台自动发现并补齐本地参考集合。源码启动可直接识别项目根目录的 `anima-ref/`；安装版可把完整 `anima-ref/` 放在 EXE 旁，或者放在当前 `workspaces.db` 所在的数据目录中。数据目录中的集合优先。参考案例页显示扫描结果，并提供“重新扫描本地案例”，可识别启动后才放入的素材。此机制不会把原始研究图片加入发行包。

扫描复用下述导入校验与保留规则，不重复创建案例，不恢复用户主动删除的案例，也不会因源目录缺失而删除库中已有资料。失败不会阻止软件启动；写入中断后可重新扫描补齐。没有联网搜索或全盘搜索。以下 CLI 仍可用于显式选择其他来源和目标。
