...
> 当前项目开发与本机使用入口已切换到 V3：双击 `启动 ANIMA V3.cmd`。当前功能与验收见 [v3/STATUS.md](v3/STATUS.md) 和 [2026-09-12 交付说明](docs/v3/NON_UI_RELEASE_20260912.md)。下方保留 V2 的历史使用及下载说明；V3 发行构建输出到 `release/`，该目录不随源码保存，也不表示已经上传公开 Releases。
...
同一项目重复双击会复用正在运行的后台并重新打开网页。后台运行期间，可把浏览器中的普通本机网址复制到其他窗口或浏览器，页面自动建立会话；后台停止后需要再次启动程序。独立发行版入口是 `AnimaPromptStudioV3.exe`，与开发版 `.cmd` 使用不同的打包方式；已有 `.exe` 不会随源码修改自动更新，需要重新打包才能包含新功能。
