...
本地优先的 ANIMA 中文提示词翻译、检查、标签化、编译和远程生图桌面工具。默认提示词处理完全在本地运行；只有用户主动打开小说画面提取助手或远程生成时，软件才会连接相应服务。
...
本工具定位为 ANIMA Prompt 创作、编译与用户自有 ComfyUI 执行客户端。它不会自动选择 Model；只有用户从「小说助手」主动提取小说画面时才调用大模型。远程执行只连接用户明确配置并确认过 SSH 指纹的云主机。
