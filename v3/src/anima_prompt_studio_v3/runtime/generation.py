...
class CandidateToPromptJobAdapter:
    """Convert a validated V3 candidate into the stable V2 execution DTO."""

    def __init__(self, config: V3RuntimeProfiles | None = None) -> None:
        self.config = config or V3RuntimeProfiles()

    def _fallback_preset(self, model_id: str, preset_id: str):
        """Only omitted fields use V3 defaults; historical labels never reset edits."""
    ...
    def prepare(
        ...
        *,
        ...
        return PreparedGeneration(job=job, checkpoint_logical_name=model.checkpoint_logical_name)
...
CandidateToV2PromptJobAdapter = CandidateToPromptJobAdapter
V2GenerationSettings = GenerationSettings
V2PreparedGeneration = PreparedGeneration
