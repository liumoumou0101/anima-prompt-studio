...
class ConfigManager:
    def __init__(self):
        ...
        # 获取 ComfyUI 用户目录
        try:
            override_dir = os.environ.get("ANIMA_PROMPT_ASSISTANT_DIR")
            user_dir = override_dir or os.environ.get("LOCALAPPDATA") or os.path.expanduser("~")
            if user_dir:
                if override_dir:
                    # 显式覆盖（测试/便携模式）
                    self.base_dir = override_dir
                else:
                    # ANIMA 用户配置目录（与 V2 数据目录保持一致）
                    self.base_dir = os.path.join(user_dir, "AnimaPromptStudio", "prompt-assistant")
            else:
                # 回退到插件目录
