...
class OpenAICompatibleService(BaseAPIService):
    ...
    async def _http_request_chat_completions(
        ...
        try:
            ...
            if urlsplit(base_url).hostname == "opencode.ai":
                # Identify the actual application; never impersonate OpenCode.
                headers["User-Agent"] = "AnimaPromptStudio/0.1"
                headers["x-opencode-session"] = cls._opencode_session
            ...
            # 前置中断检查：ANIMA 环境无 ComfyUI PromptServer，仅做中断检测（有则用）
            try:
