...
状态（2026-09-12）：V3 是当前项目主入口；非视觉功能集中开发与交付，UI 视觉调整留到后续。

当前使用根目录的 `启动 ANIMA V3.cmd` 或 V3 Windows 便携版。V3 源码位于本目录；根目录 V2 包仍提供部分兼容服务，不是当前默认产品入口。
...
从导航打开“参考案例”，即可浏览本地导入、个人收藏与已激活的内置样例。官方参考样例随 wheel 和 Windows 包携带；点击“安装内置样例”启用木刻、炭笔、水彩三图包。此操作无需联网或调用模型，已有可用官方包不会被替换，个人笔记保留；也支持命令行安装，见 [官方参考样例包](example-packs/README.md)。
...
V3 产品层通过正式包依赖复用 V2 的稳定服务。按上面的顺序开发安装，或发布时同时提供 V2/V3 wheel，安装器会解析 V2、PySide6、FastAPI 与 Uvicorn 等完整运行依赖；桌面/API 启动器默认启用运行库，新机自动初始化；`--runtime-database` 指定位置（兼容旧 `--v2-database` 参数），只有显式 `--without-runtime` 才禁用生成。
...
连接 V2 云主机配置启动后，默认 ComfyUI 的安全网页入口为 `http://127.0.0.1:18188`。也可以在“设置 → ComfyUI 网页维护入口”中为当前云主机点击打开；该入口随项目关闭，不要求开放云端公网 8188。
