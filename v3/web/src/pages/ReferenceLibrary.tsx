...
export function ReferenceLibrary({record, disabled = false, onPin, onUnpin, standalone = false, exampleId, onStart}: {
  ...
  const [catalogEnabled, setCatalogEnabled] = useState(standalone);
  ...
  const selectedRef = useRef<Example | null>(null);
  const title = useRef<HTMLInputElement>(null);
  const file = useRef<HTMLInputElement>(null);
  const sentinel = useRef<HTMLDivElement>(null);
  const catalog = useReferenceCatalog<Example>({enabled: catalogEnabled, filters: {
    q: appliedQuery, origin, model, artist, dependency, contentLevel,
  }});
  ...
  useEffect(() => {
    if (!standalone || !exampleId) return;
    const controller = new AbortController();
    void apiRequest<Example>(`/api/v3/reference-examples/${encodeURIComponent(exampleId)}`, {signal: controller.signal})
      .then(choose).catch(error => {if (!controller.signal.aborted) setMessage((error as Error).message);});
    return () => controller.abort();
  }, [standalone, exampleId]);
  useEffect(() => {
    if (!catalogEnabled || !standalone) return;
    const controller = new AbortController();
    void loadFacets(controller.signal).catch(error => {if (!controller.signal.aborted) setMessage((error as Error).message);});
    return () => controller.abort();
  }, [catalogEnabled, standalone]);
  useEffect(() => {
    if (!catalogEnabled || !page?.next_cursor || catalog.loadingMore || catalog.error || !sentinel.current || typeof IntersectionObserver === "undefined") return;
    const observer = new IntersectionObserver(entries => {
      if (entries.some(entry => entry.isIntersecting)) void catalog.loadMore();
    }, {root: null, rootMargin: "300px 0px"});
    observer.observe(sentinel.current);
    return () => observer.disconnect();
  }, [catalogEnabled, page?.next_cursor, catalog.loadingMore, catalog.error, catalog.loadMore]);
  ...
  async function loadFacets(signal?: AbortSignal) {
    const nextFacets = await apiRequest<{models?: string[]; artists?: string[]}>("/api/v3/reference-examples/facets", {signal});
    if (!signal?.aborted) setFacets({models: nextFacets.models || [], artists: nextFacets.artists || []});
  }
  ...
  async function refreshLocalReferences(refreshSelected: boolean) {
    const target = selected;
    const selectedSnapshot = JSON.stringify(target);
    await Promise.all([catalog.refresh(), standalone ? loadFacets() : Promise.resolve()]);
    if (!refreshSelected || !target) return;
    if (selectedRef.current?.id !== target.id || JSON.stringify(selectedRef.current) !== selectedSnapshot) {setLocalRefreshPending(true); return;}
    const refreshed = await apiRequest<Example>(`/api/v3/reference-examples/${encodeURIComponent(target.id)}`);
    if (selectedRef.current?.id === target.id && JSON.stringify(selectedRef.current) === selectedSnapshot) choose(refreshed);
    else setLocalRefreshPending(true);
  }
  async function refreshCatalog() {await catalog.refresh();}
  function applySearch() {
    if (query === appliedQuery) void catalog.refresh();
    else setAppliedQuery(query);
  }
  function clearFilters() {
    const alreadyClear = !query && !appliedQuery && !origin && !model && !artist && !dependency && !contentLevel;
    setQuery(""); setAppliedQuery(""); setOrigin(""); setModel(""); setArtist(""); setDependency(""); setContentLevel("");
    if (alreadyClear) void catalog.refresh();
  }
  ...
  const content = <>
    {!page && catalog.loadingFirst && <p className="library-caption" role="status">正在读取参考资料…</p>}
    {standalone && <><ReferenceLocalSync onCompleted={() => {
      if (lock.current || requirementsDirty || notesDirty || query !== appliedQuery || catalog.loadingFirst || catalog.loadingMore) setLocalRefreshPending(true);
      else void refreshLocalReferences(true).catch(error => setMessage((error as Error).message));
    }} />
    {localRefreshPending && <div className="reference-local-refresh"><span>本地案例已更新；当前编辑或筛选仍保留。</span>
      <button type="button" onClick={() => {const refreshSelected = !requirementsDirty && !notesDirty;
        if (refreshSelected) setLocalRefreshPending(false);
        void refreshLocalReferences(refreshSelected).catch(error => {if (refreshSelected) setLocalRefreshPending(true); setMessage((error as Error).message);});}}>刷新案例列表</button></div>}</>}
    <fieldset disabled={busy}>
      <div className="reference-toolbar">
      <div className="reference-search-row"><label>搜索收藏<input value={query} maxLength={200} placeholder="标题、提示词、笔记…" onChange={event => setQuery(event.target.value)} onKeyDown={event => {if (event.key === "Enter") {event.preventDefault(); applySearch();}}} /></label>
        <button type="button" onClick={applySearch}>搜索 / 刷新</button></div>
      {standalone && <div className="reference-filter-row"><label>案例来源<select value={origin} onChange={event => setOrigin(event.target.value)}><option value="">全部来源</option><option value="research_import">社区参考资料</option><option value="session_pin">生成记录</option><option value="gallery_keep">画廊图片</option><option value="upload">上传图片</option><option value="official">内置样例</option></select></label>
        <label>参考模型<select value={model} onChange={event => setModel(event.target.value)}><option value="">全部模型</option>{facets.models.map(value => <option key={value}>{value}</option>)}</select></label>
          ...
          <label>内容标记<select value={contentLevel} onChange={event => setContentLevel(event.target.value)}><option value="">全部内容</option>{Object.entries(contentLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
          <div className="reference-filter-actions"><button type="button" onClick={clearFilters}>清除筛选</button></div></div>}
      </div>
      ...
      <div className="reference-upload-form"><label>图片标题<input ref={title} maxLength={200} /></label>
        ...
        <button disabled={requirementsDirty || notesDirty} onClick={() => void act(async () => {
          ...
          choose(example); await refreshCatalog();
      ...
      {page && !page.official_pack.ready && <div className="reference-install-row">
        ...
        <button onClick={() => void act(async () => {
          ...
          await Promise.all([refreshCatalog(), loadFacets()]); setMessage("内置样例已就绪，可以选择参考并复制风格要求。");
        })}>安装内置样例</button>
      </div>}
      {page && !page.items.length && !catalog.loadingFirst && <div className="reference-catalog-state"><p>没有符合当前筛选的案例。</p><button type="button" onClick={clearFilters}>清除筛选</button></div>}
      {(requirementsDirty || notesDirty) && <p>当前案例有未保存修改，保存或重新加载后再切换案例。</p>}
      {page && <p className="reference-count" aria-live="polite">已载入 {page.items.length} / {page.total} 条参考{query !== appliedQuery ? " · 正在等待搜索" : ""}</p>}
      ...
      {page?.next_cursor && !catalog.error && <><div ref={sentinel} className="reference-catalog-sentinel" aria-hidden="true" />
        <button type="button" disabled={catalog.loadingMore} onClick={() => void catalog.loadMore()}>{catalog.loadingMore ? "正在加载更多…" : "加载更多"}</button></>}
      {catalog.loadingMore && <p className="reference-catalog-state" role="status">正在加载更多案例…</p>}
      {catalog.error && <div className="reference-catalog-state" role="alert"><p>{catalog.error.message}</p><button type="button" onClick={() => void catalog.retry()}>{catalog.error.cursorConflict ? "刷新列表" : "重试"}</button></div>}
      {page && page.items.length > 0 && !page.next_cursor && <p className="reference-catalog-state">已显示全部结果。</p>}
      </div>
      {selected && <section className="reference-detail" aria-label="参考详情">
        ...
        <button disabled={requirementsDirty} onClick={() => void act(async () => {const official = selected.origin === "official";
          ...
          method: "PATCH", body: JSON.stringify(official ? {override_revision: selected.override_revision || 0, notes: selected.notes}
            : {revision: selected.revision, title: selected.title, notes: selected.notes})}); choose(saved); await refreshCatalog();})}>{selected.origin === "official" ? "保存我的笔记" : "保存标题与笔记"}</button>
        {selected.origin === "official" ? <button disabled={notesDirty || requirementsDirty} onClick={() => void act(async () => {
          ...
          choose(copied); await refreshCatalog();
        })}>复制为我的参考</button> : <>
        ...
        <button disabled={requirementsDirty || notesDirty || !selected.notes.external_prompt.trim()} onClick={() => void act(async () => {
          ...
          choose(saved); await refreshCatalog(); setMessage(saved.warnings.join("；") || "提示词已提取，请核对要求。");
        ...
        <button disabled={requirementsDirty || notesDirty} onClick={() => void act(async () => {const saved = await apiRequest<Example & {warnings: string[]}>(`/api/v3/reference-examples/${selected.id}/ingest`, {
          method: "POST", body: JSON.stringify({revision: selected.revision, use_external_prompt_notes: sendNotes})}); choose(saved); await refreshCatalog(); setMessage(["读图建议已生成，仅供参考，请核对和修正后再钉选。", ...saved.warnings].join("；"));})}>分析图片（调用当前视觉模型）</button></details></>}
        ...
        {selected.origin !== "official" && <><ReferenceRequirementsEditor value={selected.requirements} onChange={value => setSelected({...selected,requirements:value})} />
          ...
          <button disabled={!requirementsDirty || !selected.requirements} onClick={() => void act(async () => {
            ...
            choose(saved); await refreshCatalog();
          })}>保存参考要求</button></>}
        ...
        <button disabled={!removeConfirm} onClick={() => void act(async () => {await apiRequest<void>(`/api/v3/reference-examples/${selected.id}`, {method: "DELETE", body: JSON.stringify({revision: selected.revision})}); setSelected(null); await refreshCatalog();})}>确认移除收藏</button></>}
      </section>}</div>
  ...
  return standalone ? <section className="reference-library library-reference reference-library-page">{content}</section> : <details className="reference-library library-reference" onToggle={event => {if (event.currentTarget.open) setCatalogEnabled(true);}}><summary>参考收藏 · 上传、分析与钉选</summary>{content}</details>;
}
